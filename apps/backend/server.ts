...
app.use(
  helmet({
    contentSecurityPolicy:
      NODE_ENV === "production" ? { directives: cspDirectives } : false,
...
authRouter.get("/github/callback", async (req, res) => {
  ...
  let user = await withDb(() =>
    prisma.user.findUnique({ where: { githubId: String(githubUser.id) } }),
  );
  if (!user) {
    user = await withDb(() =>
      prisma.user.create({
...
server.on("upgrade", async (request, socket, head) => {
  ...
  if (url.pathname === "/api/v1/ws") {
    ...
    const token =
      ...
      extractToken({ headers: request.headers } as any);
    if (!token) {
...
async function calculateResult(
  ...
  const interviewContextStr = context
    ? `Type: ${context.type}${context.jobRole ? `, Job Description Provided` : ""}${context.type === "GitHub" && context.githubMetadata ? `, GitHub Repos analyzed` : ""}`
    : "General interview";
  ...
  return {
    ...
    feedback: result.feedback.slice(0, 5000),
  };
}
...
app.post("/api/v1/pre-interview/github", sensitiveLimiter, async (req, res) => {
  ...
  if (
    ...
    res.status(400).json({ error: "Valid GitHub profile URL required" });
    return;
  }
...
app.get("/api/v1/dashboard/stats", sensitiveLimiter, async (req, res) => {
  ...
  res.json({
    ...
    averageScore,
    scoresOverTime: completed.map((i) => ({
...
app.get("/api/v1/analytics", sensitiveLimiter, async (req, res) => {
  ...
  const avgScore =
    scoreList.length > 0
      ? Math.round(
          (scoreList.reduce((a, b) => a + b, 0) / scoreList.length) * 10,
        ) / 10
      : 0;
  const recentAvg =
    scoreList.slice(-3).length > 0
      ? Math.round(
          (scoreList.slice(-3).reduce((a, b) => a + b, 0) /
            scoreList.slice(-3).length) *
            10,
        ) / 10
      : 0;
  const improvement =
    scoreList.length >= 2
      ? Math.round(
          ((scoreList[scoreList.length - 1] ?? 0) - (scoreList[0] ?? 0)) * 10,
        ) / 10
      : 0;

  const skillKeywords: Record<string, RegExp> = {
    "Data Structures":
      /array|linked list|stack|queue|tree|graph|hash|heap|trie/i,
    Algorithms:
      /sort|search|recursion|dp|dynamic.program|greedy|backtrack|divide|conquer/i,
    "System Design":
      /scalab|distributed|microservice|load.balanc|cache|database.shard|consistenthash/i,
    Databases:
      /sql|nosql|index|query|normaliz|transaction|acid|mongodb|postgres|mysql/i,
    "Web Dev":
      /react|api|rest|graphql|http|frontend|backend|full.stack|express|next/i,
    "Problem Solving":
      /complexity|optimize|refactor|edge.case|brute.force|efficient/i,
  };
  ...
  res.json({
    ...
    improvement,
    scoreList,
    radarData: Object.entries(skillScores)
      .map(([skill, scores]) => ({
        skill,
        score:
          scores.length > 0
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!plan || plan.price === 0 || !RAZORPAY_KEY_ID || !RAZORPAY_KEY_SECRET) {
      res
        .status(400)
        .json({ error: "Invalid plan or payments not configured" });
      return;
    }
...
Respond in JSON format: { "score": number, "keywordMatches": string[], "missingSkills": string[], "suggestions": string[], "summary": string }`;
    ...
    if (!aiRes.ok) {
      ...
      console.error("Groq error:", aiRes.status, errText.slice(0, 200));
    ...
    const keywordMatches = Array.isArray(parsed.keywordMatches)
      ? parsed.keywordMatches
          .slice(0, 50)
          .map((k: any) => String(k).slice(0, 200))
      : [];
    ...
    if (!res.headersSent) res.status(500).json({ error: "ATS check failed" });
    return;
  } finally {
    clearTimeout(t);
  }
...
app.get("/api/v1/ats/:id", sensitiveLimiter, async (req, res) => {
  ...
  const check = await withDb(() =>
    prisma.atsCheck.findFirst({ where: { id, userId: user.id } }),
  );
  if (!check) {
...
app.use(
  (
    ...
    console.error("Unhandled error:", err.message, err.stack?.slice(0, 500));
